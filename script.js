(function () {
  ...
  /* ======================================================
     FLOATING CTA — always visible, hides near the order form,
     raises above the mobile sticky bar when it appears
     ====================================================== */
  var floatingCta = document.getElementById("floatingCta");
  var orderSection = document.getElementById("order");

  if (floatingCta && orderSection && "IntersectionObserver" in window) {
  ...
  function syncFloatingCtaPosition() {
    if (!floatingCta || !stickyCta) return;
    floatingCta.classList.toggle("is-raised", stickyCta.classList.contains("is-visible"));
  }
  window.addEventListener("scroll", syncFloatingCtaPosition, { passive: true });
  ...
  /* Gallery hint: on first load, nudge the visuals so the visitor
     notices the images can be browsed */
  window.addEventListener("load", function () {
    window.setTimeout(function () {
      ...
      if (isMobileGallery) {
        ...
        slider.scrollTo({ left: step * 0.55, behavior: "smooth" });
        window.setTimeout(function () {
          slider.scrollTo({ left: 0, behavior: "smooth" });
        }, 650);
        window.setTimeout(function () { slider.classList.remove("is-hinting"); }, 1400);
      } else if (galleryMainWrap && thumbs.length > 1) {
        galleryMainWrap.classList.add("is-hinting");
        var order = [1, 2, 0];
        order.forEach(function (thumbIndex, i) {
          window.setTimeout(function () {
            var thumb = thumbs[thumbIndex];
            selectImage(thumb.getAttribute("data-full"), thumb);
          }, 350 * (i + 1));
        });
        window.setTimeout(function () { galleryMainWrap.classList.remove("is-hinting"); }, 1500);
      }
    }, 900);
  });
