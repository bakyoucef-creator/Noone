(function () {
  ...
  /* ======================================================
     BUY BAR — persistent, visible from the first moment,
     only hides once the order form itself is in view
     ====================================================== */
  var buyBar = document.getElementById("buyBar");
  var orderSection = document.getElementById("order");

  if (buyBar && orderSection && "IntersectionObserver" in window) {
    var buyBarObserver = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        buyBar.classList.toggle("is-hidden", entry.isIntersecting);
      });
    }, { threshold: 0.15 });
    buyBarObserver.observe(orderSection);
  }
  ...
  function selectImage(full, thumbEl) {
  ...
  thumbs.forEach(function (thumb) {
    thumb.addEventListener("click", function () {
      selectImage(thumb.getAttribute("data-full"), thumb);
    });
  });
  ...
  /* Gallery hint: on first load, cycle quickly through every photo
     in order (1 -> 2 -> 3 -> 4 -> back to 1) so the visitor clearly
     sees the images can be browsed */
  window.addEventListener("load", function () {
    window.setTimeout(function () {
      var isMobileGallery = window.innerWidth <= 960 && slider;
      var stepDelay = 280;

      if (isMobileGallery) {
        var slideCount = slider.querySelectorAll(".slide").length;
        var step = slider.clientWidth;
        var order = [];
        for (var s = 1; s < slideCount; s++) order.push(s);
        order.push(0);

        order.forEach(function (slideIndex, i) {
          window.setTimeout(function () {
            slider.scrollTo({ left: step * slideIndex, behavior: "smooth" });
          }, stepDelay * (i + 1));
        });
      } else if (galleryMainWrap && thumbs.length > 1) {
        galleryMainWrap.classList.add("is-hinting");
        var order2 = [];
        for (var t = 1; t < thumbs.length; t++) order2.push(t);
        order2.push(0);

        order2.forEach(function (thumbIndex, i) {
          window.setTimeout(function () {
        ...
        window.setTimeout(function () {
          galleryMainWrap.classList.remove("is-hinting");
        }, stepDelay * (order2.length + 1));
      }
    }, 800);
  });
  ...
  var buyBarPriceEl = document.getElementById("buyBarPrice");
  ...
  [priceNewEl, summaryUnitPriceEl, buyBarPriceEl].forEach(function (el) {
  ...
  function generateOrderId() {
    return "YZ-" + Date.now().toString().slice(-6);
  }

  function selectedDeliveryLabel() {
    var activeCard = document.querySelector(".delivery-card.is-active");
    return activeCard ? activeCard.querySelector(".delivery-name").textContent : "—";
  }
  ...
  orderForm.addEventListener("submit", function (e) {
    ...
    currentOrder = {
      ...
      total: subtotal + state.deliveryPrice
    };
  ...
  function onOtpSuccess() {
    ...
    state.quantity = 1;
    qtyInput.value = 1;
    document.querySelectorAll("#colorGroup .variant-btn").forEach(function (b, i) { b.classList.toggle("is-active", i === 0); });
    state.color = document.querySelector("#colorGroup .variant-btn").getAttribute("data-value");
    document.querySelectorAll(".delivery-card").forEach(function (c, i) { c.classList.toggle("is-active", i === 0); });
    state.deliveryPrice = parseInt(document.querySelector(".delivery-card").getAttribute("data-price"), 10);
    communeSelect.innerHTML = '<option value="" selected disabled>اختر الولاية أولاً</option>';
  ...
  /* ======================================================
     ...
     ====================================================== */
  var invoiceOverlay = document.getElementById("invoiceOverlay");
  var invoiceClose = document.getElementById("invoiceClose");
  var invoiceDone = document.getElementById("invoiceDone");
  ...
  function showInvoiceModal(order) {
    ...
    document.getElementById("invDate").textContent = formatDate(order.date);
    document.getElementById("invName").textContent = order.fullName;
    document.getElementById("invPhone").textContent = "+213 " + order.phone;
    document.getElementById("invWilaya").textContent = order.wilaya;
    document.getElementById("invCommune").textContent = order.commune;
    document.getElementById("invColor").textContent = order.color;
    document.getElementById("invQty").textContent = order.quantity;
    document.getElementById("invDeliveryType").textContent = order.deliveryLabel;
    document.getElementById("invSubtotal").textContent = formatDA(order.subtotal);
    document.getElementById("invDeliveryPrice").textContent = formatDA(order.deliveryPrice);
    document.getElementById("invTotal").textContent = formatDA(order.total);
  ...
  invoiceDone.addEventListener("click", closeInvoiceModal);
